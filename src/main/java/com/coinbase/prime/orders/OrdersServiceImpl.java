...
public class OrdersServiceImpl extends CoinbaseServiceImpl implements OrdersService {
  ...
  public CancelOrderResponse cancelOrder(CancelOrderRequest request) throws CoinbasePrimeException {
    return this.request(
        HttpMethod.POST,
        String.format(
            "/portfolios/%s/orders/%s/cancel", request.getPortfolioId(), request.getOrderId()),
        request,
        List.of(200),
  ...
  public ListOrderFillsResponse listOrderFills(ListOrderFillsRequest request)
    ...
    return this.request(
        HttpMethod.GET,
        String.format(
            "/portfolios/%s/orders/%s/fills", request.getPortfolioId(), request.getOrderId()),
        request,
        List.of(200),
  ...
  public ListOrderEditHistoryResponse listOrderEditHistory(ListOrderEditHistoryRequest request)
    ...
    return this.request(
        HttpMethod.GET,
        String.format(
            "/portfolios/%s/orders/%s/edit_history",
            request.getPortfolioId(), request.getOrderId()),
        request,
        List.of(200),
  ...
  public EditOrderResponse editOrder(EditOrderRequest request) throws CoinbasePrimeException {
    return this.request(
        HttpMethod.PUT,
        String.format(
            "/portfolios/%s/orders/%s/edit", request.getPortfolioId(), request.getOrderId()),
        request,
        List.of(200),
