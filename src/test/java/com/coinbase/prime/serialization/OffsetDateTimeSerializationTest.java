...
/**
 * Tests to verify OffsetDateTime serialization/deserialization Ensures SDK properly handles
 * date-time fields via Jackson JSR310 module
 */
public class OffsetDateTimeSerializationTest {
  ...
  public void testOffsetDateTimeSerialization() throws JsonProcessingException {
    ...
    assertTrue(
        json.contains("\"start_time\"") || json.contains("startTime"),
        "JSON should contain start_time field. Actual: " + json);
    assertTrue(
        json.contains("2024") || json.contains("1705318200"),
  ...
  public void testOffsetDateTimeRoundTrip() throws JsonProcessingException {
    ...
    assertEquals(
        original.getStartTime().toEpochSecond(), deserialized.getStartTime().toEpochSecond());
    assertEquals(
        original.getExpiryTime().toEpochSecond(), deserialized.getExpiryTime().toEpochSecond());
    assertEquals(
        original.getCreatedAt().toEpochSecond(), deserialized.getCreatedAt().toEpochSecond());
  }
  ...
  public void testOffsetDateTimeWithDifferentFormats() throws JsonProcessingException {
    ...
    assertEquals(order1.getCreatedAt().toEpochSecond(), order2.getCreatedAt().toEpochSecond());
  }
}
