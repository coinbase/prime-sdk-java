...
public class Main {
    ...
    public static void main(String[] args) {
        try {
            ...
            // Phase 0: Always fetch the latest spec into apiSpec/ (tracked in Git)
            logger.info("\nPhase 0: Fetching latest OpenAPI spec...");
            Path specPath = SpecFetcher.fetch(projectRoot);

            // Phase 1: Generate raw models using OpenAPI Generator
            logger.info("\nPhase 1: Generating raw models with OpenAPI Generator...");
            OpenApiGenerator generator = new OpenApiGenerator(specPath.toString(), tempDir);
            generator.generateModels();

            // Phase 2: Post-process models to match existing patterns
            logger.info("\nPhase 2: Post-processing models...");
            PostProcessor postProcessor = new PostProcessor(tempDir, outputDir, enumsDir, specPath);
            ...
            logger.info("\nNext steps:");
            logger.info("1. Review apiSpec/prime-public-spec.yaml and generated models");
            logger.info("2. Run: mvn spotless:apply && mvn test");
            logger.info("3. Commit spec + model changes together");
